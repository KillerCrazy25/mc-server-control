...
async def start_server(server, channel):
...
async def stop_server(server, channel):
...
async def restart_server(server, channel):
	subprocess.run(f'screen -S {server} -X stuff "stop^M"', shell = True)
	asyncio.sleep(15)
	subprocess.run(f'screen -S {server} -X stuff "python3 test.py^M"', shell = True)
...
@bot.command(description = "Create panel command.")
@commands.has_permissions(administrator = True)
async def createpanel(ctx : commands.Context):
	...
	try:
		...
		if any(char in msg.content for char in BLOCKED_CHARS):
